...
Status AuthenticateRedis(redisContext *context, const std::string &username, const std::string &password) {
  if (password == "") {
    return Status::OK();
  }
  ...
  if (username != "") {
    reply =
        reinterpret_cast<redisReply *>(redisCommand(context, "AUTH %s %s", username.c_str(), password.c_str()));
  } else {
    reply = 
...
Status AuthenticateRedis(redisAsyncContext *context, const std::string &username, const std::string &password) {
  if (password == "") {
    return Status::OK();
  }
  ...
  int status;
  if (username != "") {
    status = redisAsyncCommand(context, NULL, NULL, "AUTH %s %s", username.c_str(), password.c_str());
  } else {
